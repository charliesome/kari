...
{    
    ...
    ctx->variables = st_init_strtable();
    ctx->parent = NULL;
...
#define EXPOSE(fn) st_insert(context->variables, (st_data_t)#fn, (st_data_t)kari_create_native_function(context, kari_stdlib_##fn, NULL))
void kari_load_stdlib(kari_context_t* context)
{
    st_insert(context->variables, (st_data_t)"true",    (st_data_t)kari_true());
    st_insert(context->variables, (st_data_t)"false",   (st_data_t)kari_false());
    st_insert(context->variables, (st_data_t)"nil",     (st_data_t)kari_nil());
    st_insert(context->variables, (st_data_t)"$inc",    (st_data_t)kari_create_array());
...
kari_value_t* kari_call(kari_value_t* function, kari_value_t* argument, char** err)
{
    ...
    switch(function->type) {
        case KARI_FUNCTION:
            ...
            st_insert(ctx->variables, (st_data_t)((kari_function_t*)function)->argument, (st_data_t)argument);
